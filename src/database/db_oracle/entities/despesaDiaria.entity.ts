...
@Entity('V009_DESPESADIARIA', { schema: 'FINANCEIRO' })
export class DespesaDiariaEntity {
  ...
  @Column({ name: 'DTD_DESCRICAO', type: 'varchar2', length: 100, nullable: true })
  dtdDescricao: string;

  @PrimaryColumn({ name: 'TDE_ID_CODIGO' })
  tdeIdCodigo: number;

  @Column({ name: 'DTD_VALOR_MAX', type: 'number', precision: 10, scale: 2, nullable: true })
  dtdValorMax: number;

  @Column({ name: 'CARGO', type: 'varchar2', length: 16, nullable: true })
  cargo: string;

  @Column({ name: 'NOME', type: 'varchar2', length: 40, nullable: true })
  nome: string;

  @Column({ name: 'DES_DESCRICAO', type: 'varchar2', length: 60, nullable: true })
  desDescricao: string;
