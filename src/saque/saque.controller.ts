...
export class SaqueController {
  ...
  async findAll(
    ...
    @Query() params: FindParamsSaque,
  ): Promise<returnSaqueDto[]> {
  ...
  @ApiResponse({
    status: 200,
    description: 'Retorna todos as prestações',
    type: saquePrestacaoSwagger,
    isArray: true,
  })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
  @Get('prestacao')
  ...
  @ApiResponse({
    status: 200,
    description: 'Retorna um saque com a efetivo atualizado',
    type: SaqueDto,
  })
  @ApiResponse({ status: 404, description: 'Saque não encontrado' })
  @ApiResponse({ status: 500, description: 'Erro interno do servidor' })
